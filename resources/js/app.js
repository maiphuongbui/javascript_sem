...
function gameOver() {
  ...
  if (savedScores.length !== 0) { // v pripade, ze se nehraje prvni hra

    // tady bude potreba nejak pracovat s nactenym highscore

    highscores = [savedScores, scoreBoard] // to nejake spojeni poli?
  ...
  const retrievedScores = JSON.parse(localStorage.getItem('highscore'));

  for (let j = 0; j < retrievedScores.length; j++) { // !!! vzdy neni 5 sad jmen a skore, tudiz proto undefined
